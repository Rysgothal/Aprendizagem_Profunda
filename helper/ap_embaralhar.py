...
def CriarPasta(pDiretorioDestino: str):
    # Criando a pasta dentro do diretório
    os.makedirs(pDiretorioDestino, exist_ok = True) 
    
    return pDiretorioDestino
...
def SalvarArquivoCSV(pDataset, pPasta, pDados):
    ...
    for lClasseDataset in pDataset['Classe'].unique():
        CriarPasta(os.path.join(pPasta, lClasseDataset))
...
def SepararPastasDataset(pTexto: str):
    ...
    lPastaTreino = CriarPasta(os.path.join(lDiretorio, lCaminho, 'train'))
    lPastaTeste = CriarPasta(os.path.join(lDiretorio, lCaminho, 'test'))
